# 短信飞书助手 1.0.0-r24 发布说明

## 构建产物

- `release/luci-app-sms-feishu-forwarder-1.0.0-r24.apk`：MT5700M-CN、apk-tools 3、`aarch64_cortex-a53`
- `release/luci-app-sms-feishu-forwarder_1.0.0-24_all.ipk`：OpenWrt/opkg 兼容包

## r24 变更

- 常规轮询不再查询信号、小区或温度；仅在确有新短信需要转发时查询一次，并供同一批转发复用。
- 转发卡片状态区末尾增加“剩余流量（估算）”。
...
python3 packaging/test-apk.py release/luci-app-sms-feishu-forwarder-1.0.0-r24.apk
sh packaging/test-ipk.sh release/luci-app-sms-feishu-forwarder_1.0.0-24_all.ipk
git diff --check
```
